...
func ImportFromCSV(year int, month int) error {
  ...
  for _, row := range rows {
    ...
    amount, err := strconv.ParseInt(row[3], 10, 64); if (err != nil) { panic(err) }
    dateStr := strings.Replace(row[1], "/", "-", -1)
    date, err := time.Parse("2006-01-02", dateStr); if (err != nil) { panic(err) }
    content := row[2]
    memo := row[7]
    countIn, err := strconv.Atoi(row[0]); if (err != nil) { panic(err) }
    trans, err := strconv.Atoi(row[8]); if (err != nil) { panic(err) }
    // TODO: for now.
    userID := uint(1)
    walletName := row[4]
    ...
    err = payRepo.SetPay(&pay); if (err != nil) { panic(err) }
  }
